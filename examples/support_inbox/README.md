...
The email side is the [`actionmailagent`](../..) gem, two directories up —
this app is its reference implementation and depends on it by path. The gem
knows about email, conversations and agents, and nothing about tickets:
`BodyParser` (a reply chain reduced to what the customer wrote this time),
`Addressing` (the `support+<token>@` reply addresses), `InboundMessage`,
`LoopGuard` (bounces, autoresponders, lists, our own addresses, reply bursts),
`Mailbox` (the exchange, with hooks) and `Mailer` (the threading headers).

What is left to this app is the ticket-specific half:
`app/mailboxes/support_mailbox.rb` (~130 lines) maps a conversation onto a
`Ticket` and its `Reply` rows, `SupportMailer` adds the layout, and
`config/initializers/action_mail_agent.rb` sets the address, the delivery
mode and the loop limits.
...
19 tests, all against the mock provider: the three agent features with
solid_agent persistence and trace correlation, the send-draft flow, and the
email exchange end to end (threading, quote stripping, loop refusals,
handoff, duplicate delivery, draft mode). The transport layer's own unit
tests live with the gem, at the repository root.

## Notes

- The `Gemfile` tracks activeagent, solid_agent and actionagent on their
  `main` branches (pin released versions once activeagent v2 ships), and
  actionmailagent by path — this checkout.
- The Docker image is built from the repository root, not this directory,
  because of that path dependency: see the header of `Dockerfile`.
